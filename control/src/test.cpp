...
int main(int argc, char **argv)
{
  ...
  static const std::string PLANNING_GROUP = "end_effector"; /* Now we specify with what group we want work,
  ...
  target_pose.pose.position.x = target_pose.pose.position.x - 1.5; /* Basically our target pose is the same as current,  except that we want to move it a little bit along x-axis*/
  ros::Rate loop_rate(50); //Frequency
target_pose.pose.position.y = target_pose.pose.position.y - 1.5;
  while (ros::ok()){
